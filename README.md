...
当前版本将 API 配置改造为 AI 池流程，并把正反方站位从单个辩手表单中拆出：用户可以先添加多个 API，再用列表勾选参赛阵营。正反方拥有独立前置提示词，DeepSeek 可根据辩题生成简短明确的战队提示词。OCR 已建立入口和服务抽象，但真实图像文字识别仍需后续接入视觉模型或本地 OCR 引擎。
