...
- OCR 入口：提供图片 OCR 资料入口，1.0 版本保留可扩展服务层，后续可接入多模态识别或本地 OCR。
- 中文主页与日志：界面、状态、日志、导出记录均为中文。
- 动画与过渡：页面进入、消息出现、按钮按压带有轻量动画，保持 image2studio 风格的简洁、克制、清爽。
...
当前版本聚焦完整辩论闭环：多模型接入、轮流发言、人工插话队列、资料入口、上下文压缩和中文桌面/安卓界面。OCR 已建立入口和服务抽象，但真实图像文字识别仍需后续接入视觉模型或本地 OCR 引擎。
